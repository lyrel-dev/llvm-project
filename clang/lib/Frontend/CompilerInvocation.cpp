...
static bool FixupInvocation(CompilerInvocation &Invocation,
  ...
  // Note: HLSL-specific and HIP-specific option checks have been removed.
  // HLSL and HIP are no longer supported input languages; their inputs are
  // rejected in ParseLangArgs(). OpenCL strict-aliasing checks are also
  // removed since OpenCL is no longer supported.
...
#undef LANG_OPTION_WITH_MARSHALLING
  ...
  // Note: Objective-C generate arguments block has been removed since ObjC
  // is no longer a supported input language (rejected in ParseLangArgs()).
...
#include "clang/Basic/ABIVersions.def"
  ...
  // Note: HLSL validation has been removed since HLSL is no longer a supported
  // input language. HLSL inputs are rejected earlier in ParseLangArgs().
...
bool CompilerInvocation::CreateFromArgsImpl(
  ...
  ParseLangArgs(LangOpts, Args, DashX, T, Res.getPreprocessorOpts().Includes,
                Diags);
  // Note: RewriteObjC action ObjCExceptions setup and CUDA triple mapping
  // have been removed since ObjC and CUDA are no longer supported languages.
